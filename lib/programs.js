let ms = performance.now(),
    ...
    runPrograms = (objPrograms, objModelArrays) => {
        let //program = objPrograms.noshadow,
            program = objPrograms.shadow,
            shadowMapGenProgram = objPrograms.shadowgen;
        ...
        enableShadowGenUniforms(shadowMapGenProgram);

        let shadTex = createShadowTexture(),
            shadCameras = createShadowMapCameras();
        ...
        loop = () => {
            ...
            gl.useProgram(program);
            useShadowMap(shadTex[0]);

            gl.viewport(0, 0, w, h);
            clear();

            //vec3.rotateY(pointLightPosition, pointLightPosition, vec3.fromValues(0, 2, 0), 0.1);
            ...
            for (let sceneName in objModels) {
                for (let modelName in objModels[sceneName]) {
                    ...
                    model.draw(objMeshes[sceneName], program, camera, proj, false);
                }
            }
